...
describe(ArtistCard, () => {
  ...
  it("should render artist details correctly", () => {
    ...
    expect(
      screen.getByRole("button", { name: /get top songs/i })
    ).toBeInTheDocument();
  ...
  it("should call handlers when buttons are clicked", () => {
    ...
    const getSongsButton = screen.getByRole("button", {
      name: /get top songs/i,
    });
    fireEvent.click(getSongsButton);
    expect(mocSearchCommand).toHaveBeenCalledWith({
      type: "artist",
      id: mockArtist.id,
    });
  });
