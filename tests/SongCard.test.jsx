...
describe("SongCard", () => {
  const mockSong = {
    ...
    album: {
      name: "Mock Album",
      images: [{ url: "https://mockimage.com/cover.jpg" }],
    },
    duration_ms: 180000,
    popularity: 85,
  ...
  it("renders song details correctly", () => {
    ...
    expect(
      screen.getByText(`Artist: ${mockSong.artists[0].name}`)
    ).toBeInTheDocument();
    expect(
      screen.getByText(`Album: ${mockSong.album.name}`)
    ).toBeInTheDocument();
    expect(
      screen.getByText(`Popularity: ${mockSong.popularity}`)
    ).toBeInTheDocument();
  });
  it("calls for buttons handleremove , handleplay, handleaddedtoplaylist", () => {
    ...
    const addToPlayListButton = screen.getByRole("button", {
    ...
    expect(handleAddToPlaylist).toHaveBeenCalledWith(
      expect.objectContaining({
        name: "Mock Song",
        artist: "Mock Artist",
        id: "1",
